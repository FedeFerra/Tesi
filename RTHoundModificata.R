RTHound=function (testo, S = 500, L = 100, hclust.dist = 100, hclust.method = "complete", 
                  showTopN = 5) 
{
  ...
  for (l in 1:length(s)) {
    ...
    if (l > 1) {
      selectPeriodoPrima = testo[((S) * s[l] - (L + 1)):((S) * 
                                                           s[l] - 1)]
    ...
    idClusters = sapply(unique(tree), function(x) which(tree == 
                                                          x))
    for (i in 1:length(idClusters)) testo[idClusters[[i]]] = testo[idClusters[[i]][1]]
  }
  top=sort(table(testo), decreasing = T)[1:showTopN]
  if (showTopN > 0) 
